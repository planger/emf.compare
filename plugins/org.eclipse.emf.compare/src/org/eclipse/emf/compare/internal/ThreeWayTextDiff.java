...
public class ThreeWayTextDiff {
	...
	private int getNumberOfLinesInOrigin() {
		final String[] originLines = saveGet(getOrigin()).split(NL);
	...
	private class LineOperations {
		...
		private LinkedList<Diff> computeLineBasedDiff(String original, String revision) {
			return lbDiff.diff_lines_only(saveGet(original), saveGet(revision));
		}
		...
		public String apply(String original) {
			...
			return (String)lbDiff.patch_apply(patches, saveGet(original))[0];
		}
		 ...
		 * <p>
		 * Therefore, it first determines the line corresponding line in the revised version and checks for an
		 * insertion.
		 * </p>
		 * 
		 ...
		 * (i.e., the line index of the origin version).
		 * <p>
		 * Therefore, it first determines the line corresponding line in the revised version and checks for a
		 * deletion.
		 * </p>
		 * 
		 * @param originLineIndex
